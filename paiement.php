...
if (isset($_POST['payer'])) {
    ...
    $copmexp = mysqli_real_escape_string($con, $_POST['copmexp']);
    $reference = mysqli_real_escape_string($con, $_POST['reference']);
    $montant = mysqli_real_escape_string($con, $_POST['montant']);
    $query1="SELECT * FROM accounts where account_number ='$copmexp'";
    ...
    $statut = "succés";
    $sql1= "UPDATE accounts SET balance=balance - '$montant' where account_number ='$copmexp'";
...
if (isset($_POST['Transfert'])) {
  ...
  $numcompte = mysqli_real_escape_string($con, $_POST['numcompte']);
  $account_type2 = mysqli_real_escape_string($con, $_POST['account_type2']);
  $numcompteexpd = mysqli_real_escape_string($con, $_POST['numcompteexpd']);
  $montant = mysqli_real_escape_string($con, $_POST['montant']);
  $statut = "succes";
  
  
  $query1="SELECT * FROM accounts where account_type ='$account_type1' And 	account_number= '$numcompte'";
  $query_run1 = mysqli_query($con, $query1);
  $query2="SELECT * FROM accounts where account_type ='$account_type2' And account_number= '$numcompteexpd'";
  $query_run2 = mysqli_query($con, $query2);
  $num_ligne1=mysqli_num_rows($query_run1);
  $num_ligne2=mysqli_num_rows($query_run2);
 
  if($num_ligne1 > 0  && $num_ligne2 > 0){
  ...
  if($ligne2['balance']>=$montant){
  $sql1= "UPDATE accounts SET balance=balance + '$montant' where account_type ='$account_type1' And account_number= $numcompte";
  $query_run3 = mysqli_query($con, $sql1);
  $sql2="UPDATE accounts SET balance=balance - '$montant'  where account_type ='$account_type2' and account_number= $numcompteexpd";
  ...
  if ($query_run) {
     ...
     $_SESSION['message'] = "Transfert d'argent ne pas valider vérifiez les informations que vous avez saisies";
          header("Location: header.php");
     exit(0);
  }
...
//form paiement par chéque
if (isset($_POST['cheque'])) {
  $Beneficiaire = mysqli_real_escape_string($con, $_POST['recipient-name']);
  $account_number = mysqli_real_escape_string($con, $_POST['recipient-address']);
  $date = mysqli_real_escape_string($con, $_POST['payment-date']);
  $reference = mysqli_real_escape_string($con, $_POST['check-number']);
  $montant = mysqli_real_escape_string($con, $_POST['amount']);
  $query1="SELECT * FROM accounts where Numero_cheque ='$reference'";
  $query_run1 = mysqli_query($con, $query1);
  $num_ligne=mysqli_num_rows($query_run1);
  $statut = "échoué";
  if($num_ligne > 0){
    ...
    $_SESSION['message'] = "Se il vous plait Vérifier le numéro chéque";
    header("Location: header.php");
    exit(0);
  }
  if($ligne['balance']>=$montant){
  ...
  $statut = "succés";
  $sql1= "UPDATE accounts SET balance=balance - '$montant' where Numero_cheque ='$reference'";
  $query_run3 = mysqli_query($con, $sql1);
  $query = "INSERT INTO paymnets(payment_id, account_id, beneficiaire,beneficiaire_acc_num,montant,referance_no,statut,create_at) 
  VALUES (null,null,'$Beneficiaire','$account_number','$montant','$reference','$statut','$date')";
  $query_run = mysqli_query($con,$query);
  if ($query_run) {
    $_SESSION['message'] = "Processus de paiement terminé avec succès";
        header("Location: header.php");
